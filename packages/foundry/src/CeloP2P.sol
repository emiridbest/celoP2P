...
contract P2P is ERC20, ReentrancyGuard {
    ERC20 private CUSD;
    ...
    modifier onlySeller(uint256 _id, bool isSellOrder) {
        require(msg.sender == (isSellOrder ? sellOrders[_id].seller : buyOrders[_id].seller), "Unauthorized action");
        _;
    }

    modifier onlyBuyer(uint256 _id, bool isSellOrder) {
        require(msg.sender == (isSellOrder ? sellOrders[_id].buyer : buyOrders[_id].buyer), "Unauthorized action");
        _;
    }

    event OrderCreated(uint256 id, bool isSellOrder, address creator);
    ...
    function addSellOrder(uint256 _amount, uint256 _price, Fiat _fiat, uint256 _accountNumber, string memory _bank)
        public
    {
        addOrder(_amount, _price, _fiat, _accountNumber, _bank, true);
    }

    function addBuyOrder(uint256 _amount, uint256 _price, Fiat _fiat, uint256 _accountNumber, string memory _bank)
        public
    {
    ...
    function updateOrderToActive(uint256 _id, bool isSellOrder) public {
        ...
        require(msg.sender != (isSellOrder ? order.seller : order.buyer), "Cannot activate own order");
        order.isActive = true;
        if (isSellOrder) {
    ...
    function updateOrderToPaid(uint256 _id, bool isSellOrder) public nonReentrant {
        Order storage order = isSellOrder ? sellOrders[_id] : buyOrders[_id];
        require(order.isActive, "Order not active");
        require(msg.sender == (isSellOrder ? order.buyer : order.seller), "Unauthorized action");
    ...
    function cancelOrder(uint256 _id, bool isSellOrder) public {
        Order storage order = isSellOrder ? sellOrders[_id] : buyOrders[_id];
        require(msg.sender == (isSellOrder ? order.seller : order.buyer), "Unauthorized action");
        if (isSellOrder) {
            require(CUSD.transfer(order.seller, order.amount), "Transfer failed.");
        }
        order.isComplete = true;
 ...
 // Get all buy orders
    ...
    function getOrdersByFiat(Fiat _fiat, bool isSellOrder) public view returns (Order[] memory) {
    ...
    function getSellOrdersByFiat(Fiat _fiat) public view returns (Order[] memory) {
        return getOrdersByFiat(_fiat, true);
    }

    function getBuyOrdersByFiat(Fiat _fiat) public view returns (Order[] memory) {
    ...
    function myOpenSellorders() public view returns (Order[] memory) {
        uint256 count = 0;
        for (uint256 i = 0; i < sellOrders.length; i++) {
            if (msg.sender == sellOrders[i].buyer || (msg.sender == sellOrders[i].seller && sellOrders[i].isActive)) {
                count++;
            }
        }
        Order[] memory openSellOrders = new Order[](count);
    ...
    function myOpenBuyOrders() public view returns (Order[] memory) {
        uint256 count = 0;
        for (uint256 i = 0; i < buyOrders.length; i++) {
            if (msg.sender == buyOrders[i].buyer || (msg.sender == buyOrders[i].seller && buyOrders[i].isActive)) {
                count++;
            }
        }
        Order[] memory openBuyOrders = new Order[](count);
